...
Open any location in terminal on your local machine and clone the repository using following command :
```bash
git clone https://github.com/HMHarris-git/Freecodecamp_miniprojects.git

cd boilerplate_medical_data_visualizer/
```

Use following command to execute the file.
```bash
...
Results of Analysis will be saved in figures with names `catplot.png` and `heatmap.png`.

[ Note : To view the results immediately on execution uncomment `line 61` in `medical_data_visualizer.py` ]
